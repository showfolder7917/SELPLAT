...
功能：
用于调用 ffmpeg 将 MP4 视频中的音频提取为 WAV 文件。

作用：
为图形界面提供 MP4 到 WAV 的批量转换基础能力，并在转换时输出实时进度。
"""
...
SKILL_NAME = "MP4 转 WAV"
SKILL_DESC = "调用 ffmpeg 将 MP4 中的音频提取并输出为 WAV。"
...
def default_message_handler(message: str) -> None:
    print(f"[ffmpeg_mp4_to_wav] {message}", flush=True)
...
def run_ffmpeg_with_progress(
    ...
    emit_message(message_handler, "开始提取音频并输出 WAV。")
    with tempfile.NamedTemporaryFile(prefix="ffmpeg_progress_", suffix=".log", delete=False) as temp_file:
...
def inspect_output_audio(
    ...
    if stream.get("codec_name") != "pcm_s16le":
        issues.append("输出音频编码不是 pcm_s16le")
    return {
        "exists": True,
...
def run(
    ...
    if success:
        emit_message(message_handler, "WAV 输出检查通过。")
    else:
        emit_message(message_handler, f"WAV 输出发现问题：{video_check.get('issues', ['未知问题'])}")
    return {
