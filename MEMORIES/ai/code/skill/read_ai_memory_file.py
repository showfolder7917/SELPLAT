...
功能：
用于读取 `MEMORIES/ai` 目录下的文本文件，并清洗为更稳定的 AI 可读文本。

作用：
为整理 `MEMORIES/ai` 目录时提供统一读取入口，避免直接读取原文件。

适用场景：
- 读取 `MEMORIES/ai` 下的协议文件
- 读取 `MEMORIES/ai` 下的规则文件
...
# 技能说明，描述当前技能负责的动作。
SKILL_DESC = "读取 MEMORIES/ai 目录文件，并输出仅保留机器可读行的稳定文本。"
...
# 定义记忆库根目录，统一约束读取范围。
MEMORIES_ROOT = Path(__file__).resolve().parents[3]
# 定义 AI 侧目录，当前技能仅允许读取该目录。
MEMORIES_AI_ROOT = MEMORIES_ROOT / "ai"


# 定义相对路径格式化函数，便于返回稳定路径。
def to_relative_memory_path(target_path: Path) -> str:
    # 返回相对于记忆库根目录的相对路径。
    return str(target_path.relative_to(MEMORIES_ROOT))
...
def validate_ai_memory_file_path(file_path: str) -> Path:
    ...
    # 禁止读取记忆库之外的文件。
    if MEMORIES_ROOT not in target_path.parents:
        raise ValueError("仅允许读取 MEMORIES 目录下的文件。")
    # 仅允许读取 ai 侧文件。
    if target_path != MEMORIES_AI_ROOT and MEMORIES_AI_ROOT not in target_path.parents:
        raise ValueError("该读取器仅允许读取 MEMORIES/ai 目录文件。")
    # 返回校验通过的目标文件路径。
    return target_path
