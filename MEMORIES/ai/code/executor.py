...
def get_library_root() -> Path:
    """返回 ai/code 目录本身，后续所有注册表和模块路径都从这里展开。"""
...
def load_registries(library_root: Path) -> tuple[dict, dict, dict]:
    ...
    abilities_registry = load_json_file(library_root / "registry" / "abilities.json")
    skills_registry = load_json_file(library_root / "registry" / "skills.json")
    apps_registry = load_json_file(library_root / "registry" / "apps.json")
