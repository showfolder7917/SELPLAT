...
<!-- 本机会话临时启动环境：用于执行 ai/code 能力时优先采用已确认的本机 Python 解释器，并在正式任务前通过 execution_doc_manager 检查执行文档状态 -->
temporary_session_python_executable = python3
temporary_session_execution_doc_check_command = python3 MEMORIES/ai/code/executor.py execution_doc_manager '{"action":"check"}'
prefer_temporary_session_python_for_ai_code_executor = true
run_temporary_session_execution_doc_check_before_formal_task = true
