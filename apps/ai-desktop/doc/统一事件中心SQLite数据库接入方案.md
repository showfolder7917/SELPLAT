...
- 数据库迁移版本为 `0004`，除事件、流程、任务执行、审批、成员运行状态和应用运行会话外，已增加完整对话记忆、自由主题、课题来源消息关联和 Codex 页面历史对话压缩归档。
- `EventCenterFacade` 是应用唯一事件入口；既有 `BusinessAuditLog` 降为门面后的 JSONL 归档实现，并同步投影到 SQLite，不为任何人物建立旁路日志。
...
- 用户与南宫婉原文完整保存；用户原话进入后续分析时保持原样，南宫婉长回答读取前 80 个 Unicode 字符预览，但预览不得覆盖原文。
- 每轮由 AI 自由登记主题、类型和用户意图；问题中心变化时关闭旧主题并新建。用户消息下方显示意图摘要，便于用户发现理解偏差。

SQLite 负责跨人物查询、异常发现和审计事实；现有 JSON 状态仍负责重建复杂运行对象，两者职责不同，不互相伪装为备用数据库。

## 1. 文档状态与实施结论

- 文档性质：详细设计与分阶段实施记录。阶段0基础设施以及阶段1的事件、流程、审批、异常闭环和协作对话记忆已接入，当前执行统一测试；复杂运行对象仍由原 JSON 状态负责恢复。
...
migration-0003-event-handling-and-collaboration-memory.sql
```
...
- 用户与南宫婉对话原文完整保存到 `AiDesktopConversationMemory.content`；展示预览独立存入 `contentPreview`，不得替代原文。
- 每条用户消息的 AI 意图理解存入 `inferredIntent`。该字段只写简洁意图本身；南宫婉可见回答使用“我了解到您的想法是……”帮助用户核对。
- 每轮主题与类型存入 `AiDesktopConversationTopic`，均为自由文本；AI 判断问题中心改变时关闭旧主题并新建。
- 后续 AI 上下文完整读取用户原话和意图摘要；南宫婉回答只读取前 80 个 Unicode 字符预览，防止长回答挤占分析上下文。
