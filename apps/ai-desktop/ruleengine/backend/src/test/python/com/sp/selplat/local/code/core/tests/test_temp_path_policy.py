...
MAIN_CODE_ROOT = (
    PROJECT_ROOT
    / "apps/ai-desktop/ruleengine/backend/src/main/python/com/sp/selplat/local/code/core"
)
import unittest
...
RULEENGINE_ROOT = (
    PROJECT_ROOT / "apps/ai-desktop/ruleengine/backend/src/main/python/com/sp/selplat/ruleengine"
)
...
PYTHON_ENTRY_PATHS = [
    ...
    PROJECT_ROOT / "apps/ai-desktop/ruleengine/backend/src/main/python/com/sp/selplat/local/code"
    / ACTIVE_STABLE_USER_ID / "abilities/ai_rule_package_integrator.py",
    PROJECT_ROOT / "apps/ai-desktop/ruleengine/backend/src/test/python/com/sp/selplat/local/code"
    / ACTIVE_STABLE_USER_ID / "tests/test_ai_rule_package_integrator.py",
]
# Python 测试统一入口必须在发现测试模块前设置同一个缓存根。
PYTHON_TEST_RUNNER_PATH = (
    PROJECT_ROOT / "apps/ai-desktop/ruleengine/backend/src/test/python/run_tests.py"
)
