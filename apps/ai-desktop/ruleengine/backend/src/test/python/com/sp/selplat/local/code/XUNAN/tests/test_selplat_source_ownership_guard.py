...
PROGRAM_PATH = (
    PROJECT_ROOT
    / "apps/ai-desktop/ruleengine/backend/src/main/python/com/sp/selplat/local/code"
    / ACTIVE_STABLE_USER_ID
    / "abilities/selplat_source_ownership_guard.py"
)
RULE_PATH = (
    PROJECT_ROOT
    / "apps/ai-desktop/ruleengine/backend/src/main/resources/ruleengine/active-user/rules/平台"
...
class SelplatSourceOwnershipGuardTests(unittest.TestCase):
    ...
    def create_fixture(self, temp_root: Path) -> Path:
        ...
        registry = (
            temp_root / "apps/ai-desktop/ruleengine/backend/src/main/resources/local"
            / ACTIVE_STABLE_USER_ID
    ...
    def register_managed_database_application(
        ...
        registry = (
            fixture / "apps/ai-desktop/ruleengine/backend/src/main/resources/local"
            / ACTIVE_STABLE_USER_ID
            / "selplat/通用/registry/managed-database-applications.json"
        )
        registry.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_real_sel_ui_has_complete_page_editor_contract(self) -> None:
        ...
        governance_rule = (PROJECT_ROOT / "apps/ai-desktop/ruleengine/backend/src/main/resources/local/XUNAN/selplat/通用/rule/RUL_SELPLAT公共控件治理门禁规则.md") \
            .read_text(encoding="utf-8")
        self.assertIn(
    ...
    def test_rule_engine_accepts_active_user_ability_and_rejects_pollution(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="source_guard_", dir=OPTION_TEMP_ROOT) as directory:
            fixture = self.create_fixture(Path(directory))
            ability_root = (
                fixture / "apps/ai-desktop/ruleengine/backend/src/main/python/com/sp/selplat/local/code"
    ...
    def test_rule_engine_accepts_new_python_infrastructure_package(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="source_guard_", dir=OPTION_TEMP_ROOT) as directory:
            ...
            package_root = (
                fixture
                / "apps/ai-desktop/ruleengine/backend/src/main/python/com/sp/selplat/ruleengine/abilities"
            )
