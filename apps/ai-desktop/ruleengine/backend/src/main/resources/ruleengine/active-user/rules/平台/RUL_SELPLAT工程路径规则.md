...
selplat_rule_python_root = apps/ai-desktop/ruleengine/backend/src/main/python/com/sp/selplat/ruleengine

<!-- 尚未迁移的旧 core 和当前用户能力在本阶段继续从原分层根读取。 -->
selplat_rule_python_legacy_root = apps/ai-desktop/ruleengine/backend/src/main/python/com/sp/selplat/local/code

<!-- 全部派生路径统一从公共路径配置文件解析。 -->
selplat_rule_engine_path_config = apps/ai-desktop/ruleengine/backend/src/main/resources/ruleengine/config/路径配置.toml

<!-- rule-engine 的 Java 正式源码根不得重新建立。 -->
selplat_rule_engine_forbidden_source_root = apps/ai-desktop/ruleengine/backend/src/main/java
<!-- selplat_rule_engine_forbidden_source_root.2 的当前独立事实为 apps/ai-desktop/ruleengine/backend/src/main/node。 -->
selplat_rule_engine_forbidden_source_root.2 = apps/ai-desktop/ruleengine/backend/src/main/node

<!-- 规则、协议、注册信息、文档和模板统一由标准 resources 承载，并由 Python 加载器按索引直接读取。 -->
selplat_rule_resource_root = apps/ai-desktop/ruleengine/backend/src/main/resources
<!-- 新增或调整后的 rule-engine 专项规则进入新的资源命名空间。 -->
selplat_rule_engine_new_rule_root = apps/ai-desktop/ruleengine/backend/src/main/resources/ruleengine/active-user/rules
<!-- selplat_rule_resource_layer_pattern 的当前独立事实为 apps/ai-desktop/ruleengine/backend/src/main/resources/local/<layer>/。 -->
selplat_rule_resource_layer_pattern = apps/ai-desktop/ruleengine/backend/src/main/resources/local/<layer>/
...
selplat_project_common_rule_root = apps/ai-desktop/ruleengine/backend/src/main/resources/local/<layer>/selplat/通用/rule
