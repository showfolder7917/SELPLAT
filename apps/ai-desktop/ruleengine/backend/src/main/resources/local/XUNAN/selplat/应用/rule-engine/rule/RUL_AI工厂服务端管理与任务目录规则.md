...
<!-- 当前规则用于约束 SELPLAT AI 工厂服务端的登记、Gate、任务路径、审计和可视化职责。 -->
rule_scope = active_user_selplat_ai_factory_server_management_and_runtime_paths
<!-- 3.0.0 退役原本地驱动应用及其 Python、Agent 和资源维护职责。 -->
rule_version = 3.0.0
...
<!-- AI 工厂不再拥有本地工作流驱动应用；外部调用只通过已登记 HTTP 接口进入服务端校验和持久化边界。 -->
...
<!-- AI 工厂 backend 不提供 application、mainClass 或 run；所有服务端调试也必须通过平台 Host 聚合上下文。 -->
...
<!-- Java只审计其实际观察到的 API 请求、校验和状态结果，不冒充外部执行事实。 -->
...
<!-- Host 只导入 AI 工厂模块配置；AI 工厂不存在第二个 Spring Boot 启动类。 -->
...
<!-- 交付前必须验证服务端禁止能力、登记接口、审计事实和受控管理边界。 -->
