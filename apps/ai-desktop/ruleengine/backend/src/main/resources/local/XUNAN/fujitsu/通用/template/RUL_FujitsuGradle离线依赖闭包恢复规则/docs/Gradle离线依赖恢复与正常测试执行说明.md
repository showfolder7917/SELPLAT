...
直接调用当前稳定用户的 Fujitsu 能力文件，不再经过 core 注册表：

```powershell
python3 "${SELPLAT_ROOT}/apps/ai-desktop/ruleengine/backend/src/main/python/com/sp/selplat/local/code/${ACTIVE_STABLE_USER_ID}/abilities/fujitsu_gradle_offline_test_restorer.py" '<context_json>'
```
