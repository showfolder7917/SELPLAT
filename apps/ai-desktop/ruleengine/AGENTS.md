...
- 在 SELPLAT 工程内向当前稳定用户交付每轮最终回答时，必须在正文末尾追加一个 HTML 注释：`<!-- SELPLAT_CORPUS_META {"title":"AI 理解后的本轮主题","type":"AI 自由判断的主题类型","intent":"本轮用户真实意图","tags":["AI 理解后生成的标签"],"summary":"本轮 AI 最终反馈主旨，最多 300 个 Unicode 字符"} -->`。
- 标题、类型、意图、标签和摘要必须由 AI 综合本轮全部用户原话与最终实施结果后生成；禁止使用关键词规则、固定枚举或机械截断代替语义判断。
- HTML 注释只服务当前回合训练语料入库，不得在可见正文中解释；系统、开发者、工具输出、内部推理和未完成回合不得生成该元数据。
