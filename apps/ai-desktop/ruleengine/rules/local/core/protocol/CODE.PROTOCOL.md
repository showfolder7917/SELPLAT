...
- 这是 rule-engine Python core 运行层的协议文件
...
<!-- rule-engine 生产执行代码只进入扁平 Python 分层目录，禁止恢复 Java 风格包路径。 -->
production_python_code_root = apps/ai-desktop/ruleengine/python/local/
production_code_languages = python
forbid_ruleengine_backend_src_or_com_sp_selplat_layout = true
...
<!-- core、common 与用户能力在 Python 下使用相同层名。 -->
core_python_code_root=apps/ai-desktop/ruleengine/python/local/core/
common_python_code_root=apps/ai-desktop/ruleengine/python/local/common/
<!-- common Python 实体已迁入当前稳定用户层，原根只保留未来人工提升模式。 -->
common_python_code_root_status=reserved_empty
user_python_code_root_pattern=apps/ai-desktop/ruleengine/python/local/<stable-user-id>/

<!-- 机器索引作为已退役记忆库的初始迁移快照保存在 core；实体迁入 Python core 后必须同步其登记路径。 -->
registry_root=apps/ai-desktop/ruleengine/rules/local/core/registry/
