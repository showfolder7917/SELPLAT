...
<!-- 3.16.0 保留稳定入口，把路径、语言、前端、数据库和报告职责拆入独立 Python 模块。 -->
rule_version = 3.16.0
<!-- 当前版本变化固定薄入口、领域模块和导入前字节码缓存重定向。 -->
...
<!-- uniform_architecture_contract.2 的当前独立事实为 no_project_name_bypass。 -->
uniform_architecture_contract.2 = no_project_name_bypass
<!-- uniform_architecture_contract.3 的当前独立事实为 table_business_plus_capability_plus_common_for_all_projects。 -->
...
<!-- 全工程源码归属由动态当前用户 Python 能力进行可重复审计。 -->
...
<!-- 门禁内部领域模块统一位于同一当前用户能力根，入口继续负责兼容导出。 -->
