...
<!-- 专题执行群是现有专题事实的只读协作投影；群内不得复制审批、分发、恢复或验收写动作。 -->
evolution_topic_execution_group_contract = one_topic_one_business_read_only_group_timeline + SQLite_source_snapshot_and_evolution_archive_are_authoritative + source_conversation_address_deliberation_approval_distribution_execution_test_release_acceptance_and_recovery_visible + human_actor_status_owner_next_step_and_recovery_point + SQLite_perspective_topic_scoped_actor_category_keyword_filter_and_read_watermark + state_version_change_and_manual_recheck_restore_projection + controlled_person_message_reuses_existing_nangong_conversation + full_person_source_in_conversation_memory_and_group_archive_only_topic_link_ids_plus_300_preview + group_cards_deep_link_to_existing_business_pages + no_duplicate_business_state_or_parallel_mutation_route
