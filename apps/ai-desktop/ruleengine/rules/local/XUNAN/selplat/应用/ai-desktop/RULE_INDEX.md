...
<!-- 修改桌面端 Codex 进程、专题工作台树节点路由、专题审批后分发工作区、开发或发布打包路径、生产 CSS、正式窗口交互、跨平台截图、运行时路径暴露、认证、会话表达、托管职责、令狐自动开关与权威保障职责、重启恢复、新建任务入口、最低需求审核、协同阶段操作者与真实发起人、执行源码留痕、集成依赖自愈、审批或启动器时加载本规则。 -->
...
<!-- 补充或审查 AI Desktop TypeScript 文件、类型、方法、关键字段和安全边界业务注释时加载。 -->
...
<!-- v5.162.0：韩立同轮继续全部可安全验收条件，逐条件绑定动作与截图，并一次返回完整失败清单。 -->
...
<!-- v5.164.0：监控者交付后以单一应用调用封存旧卡、释放人物并退役旧树；已有正式页面通过证据时，独立验收卡直接完成归档，不进入常规派发链。 -->
...
<!-- v5.117.0：令狐旧审批链退役，普通业务审批保留；重启仍核对已加载版本及原通过证据。 -->
AI_DESKTOP_EVOLUTION_PERSISTENCE_RELEASE_RULES = local/XUNAN/selplat/应用/ai-desktop/rule/RUL_AIDesktop演化持久化与发布规则.md
<!-- 修改专题演化、对话确认后一次性完整推进、人物自身能力升级、引导安装与受控重启、持久化、自动测试或 Windows/macOS 发布入口时直接加载。 -->
