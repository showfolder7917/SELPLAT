...
<!-- 南宫婉以 AI 语义判断本轮事实是否成熟；成熟后必须用可见固定邀请建立可恢复的等待确认事实，禁止再依赖模型隐藏字段。用户回复单独“1”只启动当前一轮，并沿既有课题、审批、分发、令狐门禁和韩立真实验收线路连续推进。 -->
nangong_one_shot_complete_evolution_contract = AI_semantic_maturity_then_canonical_visible_invitation_creates_persisted_waiting_confirmation + waiting_confirmation_visible_in_nangong_conversation_and_survives_restart_until_consumed_or_replaced + exact_standalone_1_consumes_confirmation_as_conversation_to_topic_authority_not_source_write_authority + no_hidden_metadata_readiness_field + no_misleading_confirmed_intent_when_confirmation_absent + persist_one_run_and_resume_on_application_start_or_collaboration_state_change + rejected_proposal_requires_new_read_only_workspace_investigation + approval_advice_never_becomes_factual_evidence + no_material_new_evidence_means_no_new_proposal_version + correction_budget_counts_revisions_not_initial_proposal + blocked_run_resumes_same_run_topic_proposal_and_recovery_point_without_enabling_long_term_switches + blocked_task_records_one_unified_failure_and_never_directly_recovers_on_state_change + local_change_ownership_preserves_same_task_and_recovery_point_without_retry_loop + organize_topic_then_formal_hanli_fact_review_then_return_nangong_then_existing_distribution_and_collaboration_execution_then_linghu_unified_test_and_recovery_then_hanli_real_application_acceptance_then_archive + no_pause_between_internal_nodes + unavoidable_external_or_high_risk_authority_blocks_with_persisted_reason_only + one_shot_hanli_decisions_are_automatic_evidence_not_manual_preference + preserve_all_long_term_automation_switches + realtime_actor_phase_action_and_blocking_reason_in_nangong_conversation_topic_group_and_workbench_tree + no_parallel_approval_distribution_recovery_or_acceptance_route
