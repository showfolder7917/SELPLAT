...
<!-- 非页面任务由韩立只读审查实际代码是否满足客户原要求；工程质量门禁继续由令狐负责。 -->
...
hanli_role_contract = user_goal_proxy + product_architecture_and_page_layout_design_owner + requirement_structure_owner + design_review_before_dispatch + page_task_real_user_journey_acceptance + non_page_task_code_requirement_conformance_review + nangong_investigates_and_assigns_under_hanli_guidance + existing_workflow_owns_normal_transitions + linghu_owns_engineering_quality_gate_technical_fault_diagnosis_repair_and_original_stage_recovery + next_action_decision + controlled_read_only_investigation + no_implementation
...
hanli_architect_design_gate = concrete_goal_and_responsibility_before_investigation + architecture_layout_userJourney_structured_review_before_approval + exact_current_evidence_and_acceptance_criteria_references + missing_or_needs_work_returns_supplement_required_to_nangong + layout_and_userJourney_not_applicable_for_non_page_requires_reason_and_current_evidence + architecture_cannot_be_skipped + persist_design_notes_in_approval + visible_design_before_workflow_dispatch + page_actual_screenshot_and_user_path_acceptance_or_non_page_readonly_code_conformance + design_defect_to_nangong_and_technical_fault_to_linghu
