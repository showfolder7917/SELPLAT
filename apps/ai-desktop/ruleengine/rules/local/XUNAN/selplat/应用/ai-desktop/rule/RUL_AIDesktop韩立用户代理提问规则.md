...
<!-- 4.1.0 固化退役边界：后续修复不得重建隔离验收环境，也不得恢复冻结的旧验收计划。 -->
rule_version = 4.1.0

<!-- 页面型任务只在当前正式应用中按普通客户路径检查；不创建或读取第二套页面、任务时间线、测试记录或文件清单。 -->
...
<!-- 每轮结果审查都必须只读检查相关源码的结构和新手可读性；测试、异常边界和工程门禁只由令狐负责。 -->
...
<!-- 页面相关任务同时执行正式页面检查与源码审查；非页面任务只执行源码审查，不允许退回页面或代码二选一。 -->
...
<!-- 托管开启后，清楚的用户目标与设计要求直接进入既有研讨审批指派链；不取消在途工作，不扩大危险操作权限。 -->
...
<!-- 旧的无用户锚点后台流程和四个独立开关不得恢复；未开启托管时新研讨必须由用户回复 1 确认，持续运行只处理有用户证据的问题且不写训练语料。 -->
...
<!-- 韩立收到检查和排查请求后自主组织只读调查；每轮独立判断证据是否足以回答，缺口可查则继续同目标补查。 -->
...
<!-- 恢复点使用既有 SQLite 内部事实入口，只追加；失败不能占用最终结果身份，重试只恢复失败阶段。 -->
...
<!-- 韩立在调查前形成设计方向，指派前检查架构及适用的页面设计；结果阶段按任务类型选择页面体验或代码符合性验收。 -->
