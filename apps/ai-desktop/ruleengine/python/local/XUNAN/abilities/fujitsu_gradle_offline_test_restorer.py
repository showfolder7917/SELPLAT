...
# 能力标识用于统一执行器注册和结果追踪。
ABILITY_ID = "fujitsu_gradle_offline_test_restorer"
...
# 当前工程根从能力文件向上识别，禁止提交机器绝对路径。
PROJECT_ROOT = next(
    candidate
    for candidate in Path(__file__).resolve().parents
    if (candidate / "settings.gradle").is_file()
)
# 当前稳定用户只能从 AGENTS.md 的唯一声明解析，禁止从目录名称反推身份。
...
# 当前用户 Fujitsu 规则资源中的模板路径从 SELPLAT 根稳定派生，不参与目标工程识别。
...
def main(arguments: list[str] | None = None) -> int:
    """接收单个 JSON 上下文并直接执行当前用户 Fujitsu 能力。"""

    # 用户层只有一个真实运行入口时直接执行程序，不为单一能力预建注册表和二次执行器。
    raw_arguments = list(arguments if arguments is not None else sys.argv[1:])
    # 缺少或多出上下文都返回用法错误，避免把不完整参数带入文件和 Gradle 操作。
    if len(raw_arguments) != 1:
        print("Usage: python3 fujitsu_gradle_offline_test_restorer.py '<context_json>'")
        return 2
    try:
        # JSON 必须解析为对象，保持 execute 的字典输入契约。
        context = json.loads(raw_arguments[0])
        if not isinstance(context, dict):
            raise ValueError("context_json 顶层必须是对象。")
    except (json.JSONDecodeError, ValueError) as error:
        # 参数错误使用结构化结果返回，便于调用方保存阻断证据。
        print(
            json.dumps(
                {
                    "status": "blocked",
        ...
        return 2
    # 用户能力仍复用既有 execute 契约，但不再依赖 core 注册表。
    result = execute(context=context, skills={}, apps={})
    print(json.dumps(result, ensure_ascii=False, indent=2))
    # 只有完成状态返回零；依赖歧义、未知动作和测试失败必须传递非零退出码。
    return 0 if result.get("status") == "completed" else int(result.get("exit_code") or 1)
