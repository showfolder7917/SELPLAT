...
ABILITY_PATH = PYTHON_SOURCE_ROOT / "ruleengine/abilities/会话记录执行器.py"
...
class SessionTurnRecorderTests(unittest.TestCase):
    ...
    def test_protocol_and_root_index_register_standalone_three(self) -> None:
        protocol_text = (
            PROJECT_ROOT
            / "apps/ai-desktop/ruleengine/rules/local/core/protocol/USER.PROTOCOL.md"
        ).read_text(encoding="utf-8")
        root_index_text = (
            PROJECT_ROOT / "apps/ai-desktop/ruleengine/rules/RULE_INDEX.md"
        ).read_text(encoding="utf-8")
        user_rule_text = (
            PROJECT_ROOT
            / "apps/ai-desktop/ruleengine/rules/local/XUNAN/跨工程通用规则"
