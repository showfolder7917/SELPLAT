/**
 * 演化共享事实输出协议，描述议题研讨、提案、审批、分发、执行、验收和归档证据。
 *
 * 生产者：中立 Evolution 门面、状态仓库和协作记忆服务。
 * 消费者：Renderer 演进工作台、韩立审批流程和灵狐修复入口。
 * 数据方向：renderer <-> preload <-> main，自动化状态由 main 推送。
 * 本文件不再使用人物名称承载共享状态；它只描述状态与命令，不直接分配 Agent、修改规则或操作版本库。
 */
...
export interface EvolutionStateEvent {
