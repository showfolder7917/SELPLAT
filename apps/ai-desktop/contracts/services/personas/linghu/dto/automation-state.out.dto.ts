...
import type { CollaborationTaskStateValue } from "../../../workflow/index.js";
...
export interface LinghuAutomationFeedbackOutDto {
  ...
  cycle: number;
  // 模块编码说明这份反馈属于哪一种独立职责。
  module: LinghuAutomationModuleValue;
  // 关联任务 ID 让页面和审计能够反查真实执行记录。
  taskId: string;
  // 完成反馈时的结构化任务状态，禁止只靠摘要文字判断结果。
  taskState: CollaborationTaskStateValue;
  // 摘要只保存可展示结论，不包含原始推理。
  summary: string;
  // 记录时间用于恢复后判断反馈的新旧顺序。
  recordedAt: string;
...
export interface LinghuAutomaticFlowSnapshotOutDto {
  ...
  blockingKind: LinghuBlockingKindValue;
}
...
export interface LinghuAutomationStateOutDto {
  ...
  testResourceState: TestResourceCoordinatorStateOutDto | null;
  // 恢复检查点保存下一次能够安全继续的位置。
  recoveryCheckpoint: string | null;
