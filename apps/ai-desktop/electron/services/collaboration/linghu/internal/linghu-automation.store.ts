...
// 文件系统 API 分别承担读取、临时写入、原子替换和备份，不使用散落的直接覆盖写。
import { copyFileSync, existsSync, mkdirSync, readFileSync, renameSync, writeFileSync } from "node:fs";
// path 负责跨 Windows/macOS 解析状态文件父目录。
...
export class LinghuAutomationStore {
  ...
  assertTestDataCleared(): void {
    ...
    const persisted = JSON.parse(readFileSync(this.#filePath, "utf8")) as LinghuAutomationStateOutDto;
  ...
  #write(state: LinghuAutomationStateOutDto): void {
    // 首次启动时创建父目录，但不越过组合根传入的状态路径。
    mkdirSync(path.dirname(this.#filePath), { recursive: true });
    // 先写同目录临时文件，保证 rename 可以提供原子替换语义。
    const temporary = `${this.#filePath}.tmp`;
    writeFileSync(temporary, `${JSON.stringify(state, null, 2)}\n`, "utf8");
    renameSync(temporary, this.#filePath);
    // 主文件成功替换后刷新备份，为下一次损坏恢复保留完整快照。
...
function readState(filePath: string): LinghuAutomationStateOutDto | null {
  try {
    // JSON 解析成功后仍要检查可迁移版本和关键 prompts 数组。
    const value = JSON.parse(readFileSync(filePath, "utf8")) as LinghuAutomationStateOutDto;
    return [1, 2].includes(Number(value.version)) && Array.isArray(value.prompts) ? value : null;
  } catch {
    // 缺失、权限或 JSON 损坏统一返回 null，由 load 决定尝试备份或安全重建。
    return null;
  }
}
