...
export class CodexConversationCorpusIngestion {
  ...
  ingestPendingRollouts(): CorpusIngestionSummary {
    ...
    // 每个会话文件单独提交事务；一个损坏文件不会制造半条语料记录或错误检查点。
    for (const rolloutPath of listRolloutFiles(this.#sessionsRoot)) this.#ingestRollout(rolloutPath, summary);
    return summary;
  }
  ...
  async #ingestRolloutIncrementally(rolloutPath: string, summary: CorpusIngestionSummary): Promise<void> {
    ...
    const insertedCount = this.#database.transaction((connection) => {
      const statements = prepareCorpusStatements(connection);
      const inserted = writeCorpusMessages(statements, messages, now);
  ...
  #ingestRollout(rolloutPath: string, summary: CorpusIngestionSummary): void {
    summary.scannedFileCount += 1;
    const relativeSourceKey = path.relative(this.#sessionsRoot, rolloutPath).split(path.sep).join("/");
    const sourceKey = this.#policy.sourceKeyPrefix ? `${this.#policy.sourceKeyPrefix}/${relativeSourceKey}` : relativeSourceKey;
    const sourceSize = statSync(rolloutPath).size;
    const checkpoint = this.#database.withConnection((connection) => prepareCorpusStatements(connection).checkpoint.get({ $sourceKey: sourceKey }) as { sourceContentHash?: string; sourceSize?: number | bigint } | undefined);
