...
type AcceptanceSceneRuntimeState = {
  topics: Array<{ topicId: string; status: string }>;
  proposals: Array<{ proposalId: string; topicId: string; status: string; distributedTaskIds?: string[] }>;
  oneShotRun?: { topicId: string | null; proposalId: string | null; status: string; phase: string } | null;
};

/** 把应用已持久化的目标归属转换成令狐可直接使用的场景事实，避免模型在源码目录中猜测运行数据。 */
export function acceptanceSceneRuntimeFacts(
  goal: HanliComputerAcceptanceInDto,
  state: AcceptanceSceneRuntimeState,
  currentWindowAvailable: boolean,
) {
  const topic = state.topics.find((item) => item.topicId === goal.topicId);
  const proposal = state.proposals.find((item) => item.proposalId === goal.proposalId && item.topicId === goal.topicId);
  const oneShotMatches = state.oneShotRun?.topicId === goal.topicId && state.oneShotRun?.proposalId === goal.proposalId;
  return {
    source: "ai-desktop-authoritative-runtime",
    currentWindowAvailable,
    targetTopicRegistered: Boolean(topic),
    targetTopicStatus: topic?.status || null,
    targetProposalRegistered: Boolean(proposal),
    targetProposalStatus: proposal?.status || null,
    distributedTaskIds: proposal?.distributedTaskIds || [],
    currentRunMatchesTarget: Boolean(oneShotMatches),
    currentRunStatus: oneShotMatches ? state.oneShotRun?.status || null : null,
    currentRunPhase: oneShotMatches ? state.oneShotRun?.phase || null : null,
  };
}
