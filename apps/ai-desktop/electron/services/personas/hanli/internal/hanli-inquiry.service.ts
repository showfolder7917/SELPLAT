...
export class HanliInquiryService {
  ...
  async #run(request: SendPersonaConversationMessageInDto, conversationId: string, customerQuestion: string, understanding: HanliInquiryUnderstanding, decision: ConversationRoundTopicDecisionInDto): Promise<PersonaConversationOutDto> {
    ...
    try {
      ...
      const findings = parseInquiryFindings(response.text, inquiry.customerQuestion);
  ...
  async #explainForCustomer(request: SendPersonaConversationMessageInDto, inquiry: HanliInvestigationRequest, findings: ReturnType<typeof parseInquiryFindings>): Promise<string> {
    try {
...
export function parseInquiryFindings(text: string, customerQuestion: string): { status: "verified" | "unknown"; answeredQuestion: string; summary: string; evidence: { source: string; detail: string }[]; unknowns: string[] } {
  const value = JSON.parse(text.trim().replace(/^```(?:json)?\s*/u, "").replace(/\s*```$/u, ""));
  if (!value || !["verified", "unknown"].includes(value.status) || typeof value.answeredQuestion !== "string" || typeof value.summary !== "string" || !value.summary.trim() || !Array.isArray(value.evidence) || !Array.isArray(value.unknowns)) throw new Error("南宫婉调查结果格式不完整");
