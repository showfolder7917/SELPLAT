...
import type { HanliConversationOutDto, SendHanliConversationMessageInDto } from "../../../../../contracts/services/personas/hanli/index.js";
...
export class HanliConversationService {
  ...
  conversation(): HanliConversationOutDto {
    const conversationId = this.options.conversation?.activeConversationId() || null;
    return this.options.memory?.readHanliConversation(conversationId)
      || { conversationId, messages: [], updatedAt: new Date(0).toISOString() };
  }

  async send(request: SendHanliConversationMessageInDto): Promise<HanliConversationOutDto> {
    ...
    const latestHanli = [...existing.messages].reverse().find((message) => message.role === "hanli");
    if (userContent === "1" && latestHanli?.content.includes(HANLI_INTERNAL_DELIBERATION_INVITATION)) {
      ...
      const next = memory.registerHanliRound({
        conversationId: existing.conversationId,
        userMessageId: request.clientMessageId || `hanli-user-${randomUUID()}`,
        userContent,
        attachmentIds: request.attachmentIds || [],
        hanliMessageId: `hanli-message-${randomUUID()}`,
        hanliContent: reply,
        createdAt,
    ...
    const recentConversation = existing.messages.slice(-16)
      .map((message) => `${message.role === "user" ? "用户" : message.role === "nangong" ? "南宫婉" : "韩立"}：${message.content}`)
      .join("\n\n");
    ...
    const conversationId = response.threadId || chat.activeConversationId();
    if (!conversationId) throw new Error("韩立会话没有返回稳定线程标识。");
  ...
  async newConversation(): Promise<HanliConversationOutDto> {
    const chat = this.options.conversation;
    if (!chat) throw new Error("韩立自由对话尚未接入。");
    await chat.newChat();
    return { conversationId: null, messages: [], updatedAt: new Date().toISOString() };
  }
}
