import type { ConfigureEvolutionAutomationRequest, EvolutionAutomationAction, EvolutionMutationRequest, HanLiAcceptancePlan, HanLiAcceptanceRun, EvolutionState } from "../../../contracts/collaboration/evolution/index.js";
...
export interface PersonaWorkflowApplicationPort {
  ...
  setOneShotAcceptanceRunner(runner: (plan: HanLiAcceptancePlan) => Promise<HanLiAcceptanceRun>): void;
  ...
  resumeOneShotRun(): Promise<EvolutionState>;
  dispatch(proposalId: string, request?: EvolutionMutationRequest): Promise<EvolutionState>;
}
...
export class PersonaWorkflowFacade {
  ...
  setAcceptanceRunner(runner: (plan: HanLiAcceptancePlan) => Promise<HanLiAcceptanceRun>) { this.#application.setOneShotAcceptanceRunner(runner); }
  ...
  configureAutomation(request: ConfigureEvolutionAutomationRequest) { return this.#application.configureAutomation(request); }
  /** 执行启动、暂停、恢复或停止控制并保存恢复点。 */
  controlAutomation(action: EvolutionAutomationAction) { return this.#application.controlAutomation(action); }
  /** 从已保存卡点恢复同一轮，不创建第二条流程。 */
  resumeOneShotRun() { return this.#application.resumeOneShotRun(); }
  /** 审批通过后分发协作任务；Workflow 不修改人物判断。 */
  dispatch(proposalId: string, request?: EvolutionMutationRequest) { return this.#application.dispatch(proposalId, request); }
}
