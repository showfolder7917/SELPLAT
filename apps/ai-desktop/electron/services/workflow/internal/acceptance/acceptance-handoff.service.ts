...
/** 验收节点的客户摘要与可折叠技术事实必须分别传入，禁止同文案复制投影。 */
export interface AcceptanceHandoffContent {
  summary: string;
  content: string;
  detail: string;
}
...
export class AcceptanceHandoffService {
  ...
  publish(proposal: EvolutionProposalOutDto, phase: "received" | "started" | "passed" | "failed", input: string | AcceptanceHandoffContent, attemptId = proposal.proposalId): void {
    ...
    const handoff = typeof input === "string"
      ? { summary: input, content: input, detail: "" }
      : input;
