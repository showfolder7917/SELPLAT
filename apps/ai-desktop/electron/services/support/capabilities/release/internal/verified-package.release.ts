...
export function resolveVerifiedDeveloperExecutable(buildRoot: string, packageOutputRoot = path.join(buildRoot, "package", "developer")): string {
  if (process.platform !== "darwin") throw new Error("当前仅支持发布已验证的 macOS 开发版。");
  const releaseRoot = path.resolve(packageOutputRoot);
  const packageArea = path.join(path.resolve(buildRoot), "package");
  if (!releaseRoot.startsWith(`${packageArea}${path.sep}`)) throw new Error("已验证开发版目录越出受控构建根。");
