...
如果 goalJson 中 `reviewMode` 为 `post-completion-review`，当前是完成态只读复核：只能 observe、使用允许的导航点击返回任务协作群、滚动、悬停、调整验收窗口或读取任务协作群状态，然后依据新的真实截图提交 finish。不得发送消息、使用键盘、拖拽、聚焦模型或进行任何业务写入。
