...
当条件要求一次性恢复入口的键盘焦点反馈时，必须在点击“从卡点继续”前先用 `hover` 取得可用态证据，再用 `key: "Tab"` 取得聚焦态的新截图；确认焦点反馈后才可点击。一次性恢复完成后入口会按场景规则移除，不能倒序补验焦点。
...
当 goalJson 中存在 `workspaceAcceptanceFixture` 时，它是主进程已经通过当前页面添加入口完成登记并确认可见的只读验收事实，不是让你创建测试数据。先 observe 并按 `displayName` 确认本轮标签已经作为新根出现，不要再次点击添加；后续只操作该标签下的目录，不能把旧临时根当作本轮证据。仅当其 mode 为 `scenarios` 时，按 instructions 逐步展开 `slow-a`、`slow-b`、`retry-once` 与 `empty` 取得截图；`retry-once` 的首次失败应使用页面原位重试，超长名称与固定尾部目录只用于窄窗口滚动检查。涉及设置中全局不存在工作区管理卡的条件，必须使用 `scroll-settings-panel` 逐次滚动当前可见设置浮层；只有回执为 `at-boundary` 且底部的新截图可见后才能判断，夹具或页面未出现前不得把猜测当作通过。

当 goalJson 中存在 `workspaceStartupRecoveryEvidence` 时，它来自当前已打包 AI Desktop 在独立工程、独立用户目录和独立临时根中的真实子进程启动检查。只有 status 为 passed，且带标记目录、登记均已移除，未标记与主工作区均保留时，才能作为“重启后首次显示前回收”的功能证据；同时仍须在当前真实或隔离页面截图中核对工作区首屏和布局。该证据不授予路径读取权限，也不能替代其他条件的页面操作。
