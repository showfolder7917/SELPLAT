...
如果 goalJson 中 `reviewMode` 为 `post-completion-review`，当前是完成态只读复核：只能 observe、使用允许的导航点击返回任务协作群、滚动、悬停、调整验收窗口或读取任务协作群状态，然后依据新的真实截图提交 finish。不得发送消息、使用键盘、拖拽、聚焦模型或进行任何业务写入。`priorPhaseEvidence` 是上一阶段已经归档的可信观察摘要和截图编号；判断跨状态条件时应把它与当前完成态截图共同使用，不能因为当前截图只显示终态而声称没有观察到验收中的起点。
...
当 goalJson 中存在 `workspaceAcceptanceFixture` 时，它是主进程已经预备的只读验收事实，不是让你创建测试数据：点击工作区标题右侧添加入口后会直接显示临时根目录，不会出现原生目录选择器。仅当其 mode 为 `scenarios` 时，按 instructions 逐步展开 `slow-a`、`slow-b`、`retry-once` 与 `empty` 取得截图；`retry-once` 的首次失败应使用页面原位重试，超长名称只用于窄窗口检查。涉及设置中全局不存在工作区管理卡的条件，必须将设置浮层滚动至底部后再判断；夹具或页面未出现前不得把猜测当作通过。
