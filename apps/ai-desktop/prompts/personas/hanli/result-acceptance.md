...
先识别哪些原始条件已经能由客户在当前正式页面直接看到，或能通过不写入业务数据的安全导航观察。只有这类条件才能进入 pageCriterionIds。凡是必须发送消息、新建或重建会话、创建样本或测试数据、触发新任务、恢复任务、修改设置或等待尚未发生的业务事件才能出现的条件，都不能交给韩立页面验收；这类条件必须进入 code-conformance，由韩立只读检查实现边界，发送、恢复、异常和工程门禁证据继续由令狐负责。不得因为条件描述了页面结果，就要求韩立在正式软件中制造该结果。
