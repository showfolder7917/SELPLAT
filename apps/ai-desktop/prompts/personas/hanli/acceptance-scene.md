...
{"requestId":"本轮请求编号","reason":"整体取证安排","segments":[{"kind":"current-window|workspace-explorer-fixture|workspace-lifecycle-review|empty-task-group|failure-recovery-timeline|inspection-lifecycle-timeline|user-language-detail-timeline|recovery-action-lifecycle|persona-conversation-lifecycle|persona-conversation-with-task-handoff|blocked","reason":"本阶段选择理由","completionReviewRequired":false,"conditions":[{"criterionId":"criterion-1","prerequisite":"此条件成立所需的页面和数据前提"}]}]}
...
- workspace-lifecycle-review：仅当能力中包含 `workspace-cleanup-recovery` 或 `workspace-startup-recovery` 时使用，且必须紧接 workspace-explorer-fixture。程序会先用同一生命周期控制器完成临时根清理、页面同步以及已批准的受控失败恢复，再把结构化结果交给韩立，同时复用当前真实窗口确认临时根已经消失。工作区收尾和重启回收条件必须放在本场景，不能使用候选代码差异的 failure-recovery-timeline 代替。
