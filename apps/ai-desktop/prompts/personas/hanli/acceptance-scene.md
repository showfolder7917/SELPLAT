...
登记参数：`{"requestId":"本轮请求编号","kind":"场景类型","reason":"本阶段选择理由","completionReviewRequired":false,"conditions":[{"criterionId":"criterion-1","prerequisite":"此条件成立所需的页面和数据前提"}]}`。完成参数：`{"requestId":"本轮请求编号","reason":"整体取证安排"}`。
