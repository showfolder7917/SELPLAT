...
每个阶段必须唯一占用至少一条原条件，或仅关联先前已占用条件以完成尚未完成的受控场景；二者不能并存，二者都为空时拒绝。`blocked` 同样必须占用它无法覆盖的原条件，并在 prerequisite 写明缺少的受控前提。关联不产生第二份原条件结论。工具拒绝时会返回 JSON，其中 `planningContext` 是当前权威剩余状态；先据此补齐 remainingCriterionIds 和 remainingRequirementIds，再调用完成工具。登记工具的动态 Schema 是唯一字段定义；完成工具仍只接受本轮 requestId 与整体取证理由。
...
- persona-empty-conversation：目标明确要求“全新空会话”或“首次使用引导”时必须使用。程序为韩立与南宫婉分别建立消息均为空的内存会话，允许按既有页面入口发送消息、添加截图和调整窗口，但不读取、写入或新建正式人物会话。
...
- blocked：某个条件的前提不明、需要尚未提供的数据，且拆分多个阶段后仍无场景可覆盖。该阶段必须在 ownedConditions 登记受阻条件，并把缺失能力写入 prerequisite；不能提交空归属。明确缺失能力，交环境排障，不能错误选择当前窗口碰运气。
