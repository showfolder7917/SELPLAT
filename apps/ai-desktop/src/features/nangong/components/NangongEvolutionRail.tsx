...
import { evolutionMutationRequest, evolutionStatusLabel } from "../../evolution/model/evolution-workbench";
...
/**
 * 南宫婉工作区：只负责展示并发起南宫婉拥有的调查、专题和提案业务。
 *
 * 真实传参示例：当前选中“调查与研讨”、南宫婉成员资料、演化状态及当前工作区。
...
export function NangongEvolutionRail({ activeNode, member, state, workspaces, locale, selectedProposalId, databaseManaged, onState, onError }: { activeNode: EvolutionWorkspaceFlowNode; member: CollaborationMemberOutDto; state: EvolutionStateOutDto; workspaces: WorkspaceStateOutDto | null; locale: LocaleValue; selectedProposalId: string | null; databaseManaged: boolean; onState(state: EvolutionStateOutDto): void; onError(message: string): void }) {
  ...
  const toggle = (kind: "evolution" | "nangong-approval" | "linghu-approval" | "execution", enabled: boolean) => void update(() => window.desktop?.setEvolutionAutomationEnabled(kind, enabled));
  ...
  const startAutomation = async () => {
    ...
    } catch (error) { onError(readableError(error, "无法启动专题自动流程。")); }
  };
  return <aside className="person-workspace-rail nangong-workspace-rail" aria-label={`${pageTitle}页面`}>
    <header><div><span>专项演化</span><h2>{pageTitle}</h2><p>{topic ? topic.title : "先在人物自由对话中讨论，确认后由南宫婉登记专题池。"}</p></div>{showTopic && <button type="button" onClick={() => { setEditingTopicId(null); setTopicDraft({ title: "", goal: "", scope: "", exclusions: "", evidence: "", acceptanceCriteria: "" }); setTopicEditorOpen((current) => !current); }}>人工登记</button>}</header>
    ...
    {showResearch && <section className="person-rail-section" aria-label="调查与讨论说明"><p>旧式韩立后台逐轮发问流程已退役。现在请直接进入韩立或南宫婉人物页面自由讨论；形成课题后，自动化只推进已确认的专题。</p></section>}
