...
import type { ApprovalGovernanceRecord, HanLiAcceptancePlan, HanLiAcceptanceRun, Locale, NangongEvolutionState } from "../../../../contracts/desktop/desktop";
...
/** 韩立统一处理方向审批和结果验收，所有写动作继续进入专题操作协调器。 */
export function HanLiEvolutionApprovalPanel({ state, locale, view, selectedProposalId, databaseManaged, onState, onError }: { state: NangongEvolutionState; locale: Locale; view: "manual" | "automatic"; selectedProposalId: string | null; databaseManaged: boolean; onState(state: NangongEvolutionState): void; onError(message: string): void }) {
...
function latestAcceptancePlan(state: NangongEvolutionState, proposalId: string): HanLiAcceptancePlan | null {
...
function latestAcceptanceRun(state: NangongEvolutionState, planId: string): HanLiAcceptanceRun | null {
