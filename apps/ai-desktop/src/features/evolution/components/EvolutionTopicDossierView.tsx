...
export function EvolutionTopicDossierView({ dossier }: { dossier: EvolutionTopicDossier }) {
  ...
  return <section className="person-rail-section evolution-topic-dossier" aria-label="专题全生命周期原始档案">
    ...
    <EvolutionDisclosure defaultOpen label="实施、测试、发布与验收时间线">{timeline.length ? timeline.map((record) => <article key={`${record.recordId}:${record.sequenceNumber}`}><small>{formatTime(record.occurredAt, dossier.topic.locale)} · {actorLabel(record.actor)} · {categoryLabel(record.category)}</small><strong>{record.title}</strong>{archiveSummary(record.payload) && <p>{archiveSummary(record.payload)}</p>}</article>) : <p>专题尚未产生执行记录。</p>}</EvolutionDisclosure>
  </section>;
}
