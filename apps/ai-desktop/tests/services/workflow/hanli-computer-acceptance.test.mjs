...
function fixture(safe = true, sendResult = { status: "sent", composerLabel: "给韩立发送消息" }, testConsoleVisible = true, taskCollaborationState = { status: "has-topics" }, screenshotScale = 1, settingsPanelVisible = true) {
  ...
  return { inputs, boundsCalls, executedScripts, completedPersonaObservations, controller, run: (model, currentGoal = goal, workspaceEvidence, interactions = { allows: () => true, completePersonaSendingObservation: () => completedPersonaObservations.push(true) }) => controller.run(currentGoal, window, model, (text) => progress.push(text), interactions, workspaceEvidence), progress };
}
...
test("发送中观察在首张截图保存后才释放窗口私有会话", async () => {
  ...
  await f.run(async (tools) => {
    ...
    assert.equal(f.completedPersonaObservations.length, 1);
    await finish(tools, id(sent));
  });
});
