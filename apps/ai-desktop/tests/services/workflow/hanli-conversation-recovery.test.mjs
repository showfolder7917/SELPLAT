...
test("恢复记录保留回合、条目与客户消息关联，并仅由可重试状态开放重试", () => {
  ...
  assert.match(workspace, /conversation\.recovery\.retryable && <button[\s\S]*?fixedUiText\(props\.locale, controller\.recovering \? "hanliRecoveryRetrying" : "hanliRecoveryRetry"\)/);
  assert.match(fixedUiText, /hanliRecoveryRetrying: "正在重试恢复"/);
  assert.match(fixedUiText, /hanliRecoveryRetry: "重试恢复"/);
  assert.match(fixedUiText, /hanliRecoveryRetrying: "復元を再試行中"/);
  assert.match(fixedUiText, /hanliRecoveryRetry: "復元を再試行"/);
  assert.match(fixedUiText, /hanliRecoveryRetrying: "Retrying recovery"/);
  assert.match(fixedUiText, /hanliRecoveryRetry: "Retry recovery"/);
