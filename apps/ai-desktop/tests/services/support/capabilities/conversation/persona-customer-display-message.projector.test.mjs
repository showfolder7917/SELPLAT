...
test("旧 hanli-reply 只在后续段落命中稳定治理边界时迁移客户首段", () => {
  ...
  const paragraphBoundary = derivePersonaCustomerDisplayMessage({
    messageId: "hanli-reply:legacy-paragraph-request",
    messageType: "customer-visible",
    speakerType: "persona",
    content: [customerReply, inlineInternalContinuation].join("\n\n"),
  });
  assert.deepEqual(paragraphBoundary, { state: "ready", content: customerReply, failureReason: null });
