...
test("首次初始化建立版本表并在重复启动时保持幂等", () => {
  ...
  try {
    ...
    assert.equal(first.status.schemaVersion, "1035");
    ...
    try {
      ...
      assert.equal(Number(row.count), 36);
      ...
      assert.deepEqual({ versionCode: version.versionCode, successFlag: Number(version.successFlag) }, { versionCode: "1035", successFlag: 1 });
      ...
      assert.ok(inspection.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='AiDesktopPersonaConversationRecovery'").get());
      assert.ok(inspection.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='AiDesktopPersonaConversationCodexThread'").get());
      assert.ok(inspection.prepare("SELECT 1 FROM pragma_table_info('AiDesktopPersonaConversationRecovery') WHERE name='affectedMessageId'").get());
    } finally {
      inspection.close();
    }
  } finally {
...
test("候选包可用自身迁移清单升级仍停在旧版本的受控工程数据库", () => {
  ...
  try {
    ...
    assert.equal(upgraded.status.schemaVersion, "1035");
    ...
    assert.ok(upgraded.database?.withConnection((connection) =>
      connection.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='AiDesktopPersonaConversationRecovery'").get(),
    ));
    assert.ok(upgraded.database?.withConnection((connection) =>
      connection.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='AiDesktopPersonaConversationCodexThread'").get(),
    ));
...
test("新写入和打开 v6 人物实现短文时均保留失败位置，不能重新显示未能安全分离的内部正文", () => {
  ...
  try {
    ...
    initialized.database?.withConnection((connection) => connection.prepare(`
      UPDATE AiDesktopPersonaCustomerDisplayMessage
      SET displayState='ready', displayContent=$raw, failureReason=NULL, derivationVersion=6
    ...
    assert.doesNotMatch(window.messages[0].content, /contentRole|持久化|恢复|页面投影/u);
    const version = initialized.database?.withConnection((connection) => connection.prepare(`
...
test("显式重建 v11 旧 hanli-reply 混合记录并保留原始审计正文", () => {
  ...
  try {
    ...
    const source = initialized.database?.withConnection((connection) => connection.prepare(`
      SELECT content FROM AiDesktopPersonaConversationMessage WHERE messageId='hanli-reply:legacy-request'
    `).get());
    assert.equal(source?.content, raw);
    const display = initialized.database?.withConnection((connection) => connection.prepare(`
      SELECT displayState, derivationVersion FROM AiDesktopPersonaCustomerDisplayMessage
      WHERE sourceMessageId='hanli-reply:legacy-request'
    `).get());
    assert.equal(display?.displayState, "ready");
    assert.equal(display?.derivationVersion, PERSONA_CUSTOMER_DISPLAY_DERIVATION_VERSION);
  } finally {
    initialized.database?.close();
    rmSync(fixture.projectRoot, { recursive: true, force: true });
...
test("1026 至 1032 升级演化快照、人物消息与自动入库恢复状态后写回 v10", () => {
  ...
  try {
    ...
    assert.equal(Number(persisted.stateVersion), 10);
    assert.equal(JSON.parse(String(persisted.stateJson)).version, 10);
    const definition = upgraded.database?.withConnection((connection) => connection.prepare(
      "SELECT sql FROM sqlite_master WHERE type='table' AND name='AiDesktopEvolutionState'",
    ).get());
    assert.match(String(definition?.sql), /stateVersion IN \(8, 9, 10\)/);
    upgraded.database?.close();
  } finally {
    rmSync(fixture.projectRoot, { recursive: true, force: true });
  }
