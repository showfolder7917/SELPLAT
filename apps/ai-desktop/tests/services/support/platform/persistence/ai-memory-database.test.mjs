...
test("首次初始化建立版本表并在重复启动时保持幂等", () => {
  ...
  try {
    ...
    assert.equal(first.status.schemaVersion, "1019");
    ...
    try {
      ...
      assert.equal(Number(row.count), 20);
      ...
      assert.deepEqual({ versionCode: version.versionCode, successFlag: Number(version.successFlag) }, { versionCode: "1019", successFlag: 1 });
