...
test("首次初始化建立版本表并在重复启动时保持幂等", () => {
  ...
  try {
    ...
    assert.equal(first.status.schemaVersion, "1028");
    ...
    try {
      ...
      assert.equal(Number(row.count), 29);
      ...
      assert.deepEqual({ versionCode: version.versionCode, successFlag: Number(version.successFlag) }, { versionCode: "1028", successFlag: 1 });
...
test("候选包可用自身迁移清单升级仍停在旧版本的受控工程数据库", () => {
  ...
  try {
    ...
    assert.equal(upgraded.status.schemaVersion, "1028");
...
test("1026 升级演化快照、1027 补齐消息类型、1028 补齐内容角色后由应用层写回 v9", () => {
