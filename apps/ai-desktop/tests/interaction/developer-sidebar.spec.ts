...
test("南宫婉可对话讨论并由韩立分别控制两个来源的自动审批", async ({}, testInfo) => {
  ...
  await workspace.getByRole("button", { name: "专题执行群", exact: true }).click();
  await expect(workspace.getByLabel("专题执行群页面")).toBeVisible();
  ...
  await workspace.getByRole("button", { name: "专题执行群", exact: true }).click();
  const topicGroup = workspace.getByLabel("专题执行群页面");
  await expect(topicGroup.getByRole("heading", { name: "南宫婉完整审批链路", exact: true })).toBeVisible();
  ...
  await expect(topicGroup.getByText("专题群收到用户消息与南宫婉回复", { exact: true })).toBeVisible();
