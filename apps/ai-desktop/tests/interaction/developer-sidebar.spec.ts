...
test("南宫婉可对话讨论并由韩立分别控制两个来源的自动审批", async ({}, testInfo) => {
  ...
  const expandedRailBounds = await rail.boundingBox();
  if (!initialRailBounds || !expandedRailBounds) throw new Error("南宫婉右侧工作栏缺少可视边界。");
  expect(expandedRailBounds.width).toBeGreaterThan(initialRailBounds.width);
  ...
  await page.getByRole("button", { name: "确认形成课题" }).click();
  ...
  await approvalPanel.getByLabel("审批建议").fill("补充任务完成记录的验收事实");
  await approvalPanel.getByRole("button", { name: "退回补充" }).click();
  await expect(approvalPanel.getByText(/用户 · supplement-required · 补充任务完成记录的验收事实/)).toBeVisible();
