...
import { registerLanguageSettingsAcceptanceScenarios } from "./language-settings-acceptance.scenario";
...
registerLanguageSettingsAcceptanceScenarios(() => ({ application, page }));
...
test("完成态复核卡点在原卡显示继续入口且恢复开始后立即消失", async () => {
  await page.evaluate(async () => {
    const api = (window as any).desktop;
    await api.setInteractionTaskTimelineFixture(true);
    await api.setInteractionAcceptanceTimelineFixture("completed");
    const now = new Date().toISOString();
    await api.setInteractionOneShotRun({
      runId: "completion-review-fixture",
      topicId: "interaction-timeline",
      proposalId: "interaction-timeline-proposal",
      status: "blocked",
      phase: "blocked",
      actor: "han-li",
      actorName: "韩立",
...
test("生产构建在正式默认、实际复现和最小窗口中保持设置入口与面板定位", async () => {
  ...
  const sizes = [
    ...
    { name: "正式最小", width: minimumWidth, height: minimumHeight },
  ];
...
test("韩立排查显示真实阶段并从原阶段重试，保留输入草稿和原问题", async ({}, testInfo) => {
  ...
  try {
    ...
    // 本用例创建的未发送草稿由本用例清理，避免共享 Electron 窗口把它带入下一条交互验证。
    await page.getByRole("textbox", { name: "给韩立发送消息" }).fill("");
    await page.evaluate(async () => {
      const api = (window as any).desktop;
