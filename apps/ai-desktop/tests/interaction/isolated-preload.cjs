...
const synchronizeInteractionCurrentTopicStage = () => {
  ...
  if (acceptanceTimelineFixtureStatus) {
    ...
    evolutionState.currentTopicStage = {
      ...
      // 验收进行中与正式阶段投影一致：保留等待原因，但不签发恢复权限。
      remaining: accepting ? "等待真实验收结果。" : "", waitingFor: accepting ? "韩立真实验收" : "", nextAction: accepting ? "等待韩立记录本轮真实验收结果，并保留原始页面证据。" : "本专题已完成",
...
contextBridge.exposeInMainWorld("desktop", {
  ...
  openWorkspaceFile: async (workspaceId, relativePath) => ({ kind: "preview", workspaceId, relativePath, content: "" }),
