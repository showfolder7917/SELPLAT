...
test("当前用户 AI Desktop 规则已登记侧栏单区独占与标签新建入口契约", () => {
  ...
  assert.match(aiDesktopRule, /rule_version\s*=\s*5\.37\.0/);
  assert.match(aiDesktopRule, /developer_sidebar_section_disclosure_contract\.3\s*=\s*workspace_tasks_single_active/);
  assert.match(aiDesktopRule, /developer_sidebar_section_disclosure_contract\.4\s*=\s*new_task_action_in_codex_chat_tab_before_close/);
  ...
  assert.match(component, /className="tab-new-task"[^>]*onClick=\{\(\) => void startNewTask\(\)\}[^>]*><Add24Regular \/><\/button>/);
