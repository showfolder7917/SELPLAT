...
test("设置业务内容区具有稳定剩余高度并独立纵向滚动", () => {
  assert.match(styles, /\.dev-settings-control > \.dev-settings\s*\{[^}]*height:\s*min\(680px, calc\(100vh - 60px\)\)/);
  assert.match(styles, /\.dev-settings-control > \.dev-settings \.selfloating-heading\s*\{[^}]*flex:\s*0 0 54px/);
  assert.match(styles, /\.dev-settings-control \.dev-settings-content\s*\{[^}]*min-height:\s*0;[^}]*flex:\s*1 1 auto;[^}]*overflow-y:\s*auto/);
  assert.match(styles, /\.dev-settings-control \.dev-settings-scroll-stack\s*\{[^}]*min-height:\s*calc\(100% \+ 1px\);[^}]*padding:\s*14px 18px 32px/);
});
