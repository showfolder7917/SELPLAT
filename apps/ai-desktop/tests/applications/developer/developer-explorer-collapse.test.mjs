...
test("当前用户 AI Desktop 规则已登记侧栏单区独占与标签新建入口契约", () => {
  ...
  assert.match(workspaceRuntimeRule, /developer_sidebar_section_disclosure_contract\.4\s*=\s*refresh_conversation_action_beside_selui_tabs_with_localized_rebuild_session_tip/);
  ...
  assert.match(component, /className="developer-page-actions"/);
  assert.doesNotMatch(component, /with-workspace-action|openEvolutionWorkspace/);
  assert.match(component, /SelUiWorkspaceTabs/);
  ...
  assert.match(styles, /\.developer-page-actions \.tab-new-task\s*\{/);
