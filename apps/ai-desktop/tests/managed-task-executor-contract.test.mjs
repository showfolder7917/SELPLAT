...
test("测试托管独立执行构建、构建后测试和至多一次受控重启", () => {
  ...
  assert.match(executor, /构建 → 构建后的针对性测试/);
...
test("多轮托管保留上一轮回答并只校准当前轮完成文本", () => {
  assert.match(developerApp, /turnTextStart\?: number/);
  assert.match(developerApp, /message\.text\.slice\(0, turnTextStart\)/);
  assert.match(developerApp, /turnTextStart: text\.length/);
