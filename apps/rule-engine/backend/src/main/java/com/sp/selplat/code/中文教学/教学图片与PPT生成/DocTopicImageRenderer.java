...
import com.sp.selplat.code.中文教学.教学图片与PPT生成.DocTopicContentRepository.TopicContent;
import com.sp.selplat.code.中文教学.教学图片与PPT生成.DocTopicPoem.AnnotatedLine;
import com.sp.selplat.code.中文教学.教学图片与PPT生成.DocTopicPoem.AnnotatedToken;
...
public final class DocTopicImageRenderer {
    ...
    // 业务上仅从字体作者维护的官方发行地址补齐缓存，避免下载来源不明的字体二进制。
    private static final String DISPLAY_FONT_DOWNLOAD_URL =
    ...
    private static final Path DISPLAY_FONT_CACHE_PATH = resolveCurrentProjectRoot()
        ...
        .normalize();
    // 业务上优先加载本地缓存字体，缓存缺失时才从官方来源补齐，确保 Windows 与 macOS 使用同一字形。
    private static final Font DISPLAY_BASE_FONT = loadCachedDisplayFont();
    ...
    /**
     * 返回当前实际使用的展示字体族，供跨平台测试确认资源字体已加载。
     */
    ...
    /**
     * 从当前工程缓存加载开源中文文楷字体；缓存缺失时按需从官方发行源补齐。
     */
    private static Font loadCachedDisplayFont() {
        try {
            // 业务上先保证缓存文件存在，避免每次教学图片生成都重复下载大体积字体。
            ensureDisplayFontCached();
            ...
            // 业务上网络不可用或字体损坏时使用可读宋体兜底，让图片生成主流程仍能给出确定结果。
            return new Font("Songti SC", Font.PLAIN, 12);
        }
    }

    /**
     * 仅在缓存缺失时下载字体，并以临时文件写入避免半下载文件被后续任务误当作有效字体。
     *
     * @throws IOException 创建缓存、下载或原子替换失败
     */
    private static void ensureDisplayFontCached() throws IOException {
        // 业务上已有可读字体缓存时直接复用，控制网络请求和重复的 25MB 左右下载。
        if (Files.isRegularFile(DISPLAY_FONT_CACHE_PATH) && Files.size(DISPLAY_FONT_CACHE_PATH) > 0) {
            return;
        }
        // 业务上先创建缓存父目录，保证首次在新工程运行时无需人工准备文件夹。
        Files.createDirectories(DISPLAY_FONT_CACHE_PATH.getParent());
        // 业务上下载写入同目录临时文件，只有完整传输后才替换正式缓存。
        Path temporaryPath = Files.createTempFile(DISPLAY_FONT_CACHE_PATH.getParent(), "LXGWWenKai-", ".part");
        try {
            // 业务上下载连接设置超时，避免网络异常时长期阻塞教学图片批处理。
            URLConnection connection = URI.create(DISPLAY_FONT_DOWNLOAD_URL).toURL().openConnection();
            connection.setConnectTimeout(15_000);
            connection.setReadTimeout(60_000);
            // 业务上只接受官方响应流，并将其完整复制到临时文件再发布。
            try (InputStream inputStream = connection.getInputStream()) {
                Files.copy(inputStream, temporaryPath, StandardCopyOption.REPLACE_EXISTING);
            }
            // 业务上下载结果为空时阻止覆盖旧缓存，避免把网络错误页当作字体文件。
            if (Files.size(temporaryPath) == 0) {
                throw new IOException("字体下载结果为空: " + DISPLAY_FONT_DOWNLOAD_URL);
            }
            // 业务上完整字体一次性替换缓存文件，后续并发任务只会读取完整版本。
            Files.move(temporaryPath, DISPLAY_FONT_CACHE_PATH, StandardCopyOption.REPLACE_EXISTING);
        } finally {
            // 业务上下载失败时删除半成品，避免下次误判缓存已经存在。
