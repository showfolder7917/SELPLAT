...
功能：
在 rule-engine Python core 中把 USER.PROTOCOL 的 1/2 确认规则落成最小可执行守门。
...
def _state_path(context: dict[str, Any]) -> Path:
    ...
    # 从迁移后的深层包向上识别当前工程根，确认状态不得写入规则引擎模块内部。
    project_root = next(
        candidate
        for candidate in Path(__file__).resolve().parents
        if (candidate / "settings.gradle").is_file()
    )
    # 当前线程的临时确认状态继续写入工程 OPTION/temp，保持可清理且不污染源码。
    state_root = project_root / "OPTION" / "temp" / "user_confirmation_gatekeeper"
