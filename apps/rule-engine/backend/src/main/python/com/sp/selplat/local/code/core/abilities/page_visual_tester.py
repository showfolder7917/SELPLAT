...
# 从迁移后的深层包向上识别项目根目录，使测试证据继续归属当前工程。
WORKSPACE_ROOT = next(
    candidate
...
"""
    # 浏览器桥接脚本和配置必须进入当前工程 OPTION/temp，不使用系统临时目录。
    DEFAULT_OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    # 自动清理的临时目录显式归属页面测试输出根，避免并发工程互相污染。
    with tempfile.TemporaryDirectory(dir=DEFAULT_OUTPUT_ROOT) as temp_dir:
