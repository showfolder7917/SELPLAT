...
- Fujitsu 工程按项目名前缀识别；项目专属细节继续进入 `src/main/resources/rule/fujitsu/<project>/`，不得全部提升为组织级规则。
...
fujitsu_shared_rule_root = apps/rule-engine/backend/src/main/resources/rule/fujitsu/
fujitsu_project_specific_rule_root = apps/rule-engine/backend/src/main/resources/rule/fujitsu/<project>/
