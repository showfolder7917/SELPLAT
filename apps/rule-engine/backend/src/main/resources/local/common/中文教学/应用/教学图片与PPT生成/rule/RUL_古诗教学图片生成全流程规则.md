...
- 无文字底图的生成、重试、临时文件迁移和专项验收，按需引用同目录的`RUL_古诗无文字底图生成工作流程规则.md`；本规则不重复维护该专项细节。
...
<!-- 核定版 DOCX 必须由 Apache POI 和既有 PinyinPoetryDocxParser 读取；适用于标题、朝代作者、逐字拼音和原文；业务含义是禁止重新调用拼音库或让模型猜测核定内容 -->
poetry_source_parse_tool = java21_apache_poi_pinyin_poetry_docx_parser
...
chinese_teaching_brand_icon_asset = local/common/中文教学/通用/template/RUL_中文教学公共品牌素材使用规则/新思度华文学堂.png
...
poetry_minimum_font_sizes_at_1053x1493 = title:64,section:30,poem_hanzi:42,pinyin:18,body:24,footer:24
...
poetry_anchor_map_required_fields = left,top,right,bottom,padding_left,padding_top,padding_right,padding_bottom
...
poetry_visual_evidence_regions = full,header_title_author,poem,body,core_life,footer
...
poetry_image_delivery_report_fields = source_docx,content_json,art_assets,fonts,render_tool,output_paths,page_count,validation_evidence
...
poetry_failure_record_required_fields = sequence,title,page,attempt_count,attempt_reasons,failure_stage,draft_paths,last_validation_evidence,recommended_manual_action,recorded_at
...
poetry_batch_archive_required_contents = final_images,page_order_manifest,generation_manifest,final_validation_report
...
poetry_batch_archive_forbidden_contents = drafts,failed_versions,visual_check_crops,temp_fonts,caches,source_docx,unrequested_source_art
...
poetry_batch_delivery_report_fields = archive_path,archive_file_count,archive_integrity_result,archive_sha256
...
poetry_workspace_responsibility_directories = <当前临时目录>/PPT排版数据（可由DOCX重新生成） + 公共/工具 + local/common/中文教学/应用/教学图片与PPT生成/rule
