...
```text
src/main/
├─ java/com/sp/selplat/local/code/common/fujitsu/
│  ├─ db/                       DB 规则工具
│  └─ sql/                      SQL 规格书工具
└─ resources/fujitsu/
   ├─ rule/                     Fujitsu 模块规则
   ├─ docs/                     使用文档
   └─ template/sql/SQL仕様書生成ツール/  SQL 规格书配置与参考模板
```
...
```text
com.sp.selplat.local.code.common.fujitsu.db.DBデータ生成ツール
```
...
```text
com.sp.selplat.local.code.common.fujitsu.sql.SQL仕様書生成ツール
```

默认配置位于 `resources/fujitsu/template/sql/SQL仕様書生成ツール/SQL仕様書生成ツール新規.json`，参考模板位于其 `reference/` 子目录，生成结果默认进入 `OPTION/temp/sql-spec`。详细配置见同目录的《SQL仕様書生成ツール使用说明》。
