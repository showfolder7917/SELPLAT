...
<!-- 正式规则在标准 resources 内按 core、common 或稳定用户标识分层；适用于分层迁移完成后的全部规则维护。 -->
rule_layer_root_pattern = apps/rule-engine/backend/src/main/resources/local/<layer>/
rule_layer_values = core,common,<stable-user-id>
...
<!-- 问题：common 中多个工程、组织和业务域全部平铺到根索引后，任何局部维护都会扩大根索引冲突和审查范围。 -->
<!-- 场景：local/common 下存在 SELPLAT、Fujitsu、中文教学及未来新增的独立规则作用域。 -->
<!-- 业务含义：每个作用域维护自己的权威索引，上级索引只负责汇总子索引，根索引保持稳定全局入口。 -->
common_first_level_directory_semantics = independent_rule_scope
common_rule_scope_examples = fujitsu,selplat,中文教学

<!-- 跨工程通用规则是所有工程可按需使用的共享作用域，不等同某个具体项目，也不得因适用范围广而默认全量加载。 -->
cross_project_common_scope = local/common/跨工程通用规则/
cross_project_common_index = local/common/跨工程通用规则/RULE_INDEX.md
cross_project_common_loading_policy = matched_rules_only,no_bulk_loading

<!-- common 汇总索引只登记一级作用域索引；禁止复制子索引内的规则逻辑 ID。 -->
...
<!-- 根索引直接登记冻结 core 规则并只汇总 common 总索引；core 平铺指索引登记，不得移动 core 实体文件。 -->
...
hierarchical_rule_change_sync_chain = owning_leaf_index -> parent_scope_indexes -> common_aggregate_index -> root_RULE_INDEX

<!-- 分层迁移期间旧目录只作为迁移输入读取；新规则和新能力不得继续写入旧布局，业务含义是过渡期不扩大待迁移范围。 -->
legacy_unlayered_rule_layout_policy = read_for_migration_only,no_new_authoring
...
rule_engine_language_native_source_roots = ../python/com/sp/selplat/local/code/<layer>/,../node/com/sp/selplat/local/code/<layer>/
