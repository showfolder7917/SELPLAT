...
<!-- 一旦判定为能力缺口，必须明确提示应升级哪类能力，例如事实抽取、模板路由、内容渲染、输出校验或规则加载；业务含义是避免只说“补能力”却不说明升级落点 -->
...
<!-- 模板骨架问题必须先修结构、锚点映射和布局骨架，不得直接在成品上长期手工绕过；业务含义是保证重跑可复现 -->
...
<!-- 每次生成器升级都必须保留其他类型扩展口，不得因为实现一个专项生成器就封死后续分流。 -->
generator_upgrade_must_keep_extension_ports = true

<!-- 升级生成器时必须同步核对能力代码、专项规则、协议、索引和注册信息；业务含义是防止只修一层而留下断裂调用链。 -->
generator_upgrade_must_sync_code_rule_protocol_index_and_registry = true

<!-- 生成器修复必须采用最小边界修复：先修抽象层，再修专项层，再修成品；业务含义是减少无关回归 -->
generator_repair_must_follow_minimal_repair_boundary = true

<!-- 自动升级或修复完成后必须验证代码可编译、能力可返回、注册可解析且规则可从索引命中；业务含义是禁止只改文本后宣称修复完成。 -->
...
7. 若属于模板骨架层，先修结构、锚点映射和布局骨架。
8. 若属于成品输出层，重跑结构检查、残留词检查和关键页截图验证。
9. 若属于规则收敛层，先合并近义规则并清理旧规则引用，再继续验证。
10. 修复完成后，必须把修复结果沉淀回协议、规则、能力代码或注册信息，而不是只修当前输出。

## 验证闭环（Verification Closure）

1. Python 能力代码升级后，至少执行 `py_compile` 或等价编译验证；其他语言执行对应的离线编译或静态验证。
2. 能力注册文件升级后，必须验证注册文件可解析、目标能力键存在且登记路径真实可达。
3. 规则索引升级后，必须验证新入口能够命中；发生合并或删除时，还必须确认旧入口和旧引用已经清理。
4. 生成器支持多模式分流时，必须验证目标模式正确执行，并验证非目标模式返回正确扩展口或阻塞信息，禁止误走默认模式。
5. 修复由截图、预览图或等价可视化反馈触发时，验证结果必须保留对应证据，并说明修复已经回写到能力、规则、模板或验证链中的哪一层。
6. 判定存在能力缺口时，验证结果必须记录能力类型、实际修改或待修改文件，以及没有选择其他能力类型的原因。
7. 只有主路径、边界路径和受影响回归路径均形成与改动风险匹配的证据后，才能宣称升级或自愈完成。

## 适用口径（Scope）

- 专项生成器必须加载自身已登记规则；若未实现专属能力，必须暴露扩展口和缺口清单，不得伪装成功。
