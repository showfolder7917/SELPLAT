...
- 这是规则引擎本地分层文件编辑场景的核心规则文件
- 本文件承接初始迁移资料及规则文本编辑时的完整读取与保真约束；core 冻结后不得自动修改
- 本文件使用结构化规则写法，不重复声明启动层和协议层已声明约束

## 强制规则（Mandatory）

<!-- 编辑允许写入的 common 或当前用户层文件时必须使用完整读取能力 -->
...
<!-- 从用户层提升为公共规则或代码必须由人工审查并手工合入 common -->
