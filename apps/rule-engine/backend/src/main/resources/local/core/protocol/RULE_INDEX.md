...
- 本文件不重复登记 common 或用户层的具体规则，只保留正式根索引入口和 core 基线规则兼容标识。
- 本文件不默认触发全量读取，规则必须按任务类型按需最小进入。
- 工程项目任务默认先按 `CODE.PROTOCOL.md` 进入语言原生 core 能力系统，再根据正式根索引加载最少必要规则。
...
RULE_ENGINE_PROJECT_RESOURCE_INDEX = ${RES}RULE_INDEX.md
...
<!-- 分层治理和语言原生实体迁移已完成；common 通过汇总与当前作用域索引递归进入，core 保持根索引直登和只读冻结。 -->
rule_engine_layer_migration_status = migration_complete_core_frozen
rule_engine_target_loading_order = local/core -> local/common/跨工程通用规则 -> local/common/matched_scope -> local/active_user
rule_engine_target_conflict_priority = local/active_user > local/common/matched_scope > local/common/跨工程通用规则 > local/core
rule_engine_common_index_chain = ${RES}RULE_INDEX.md -> ${RES}local/common/RULE_INDEX.md -> ${RES}local/common/<matched_scope>/RULE_INDEX.md
rule_engine_layer_cutover_must_follow_formal_index = true

<!--迁移期执行文档约束入口：当任务涉及执行文档启动检查时，按需读取 COMMAND.PROTOCOL.md 中的临时保留边界-->
...
standard_ability_for_page_visual_test_migration_status = active_python_core
