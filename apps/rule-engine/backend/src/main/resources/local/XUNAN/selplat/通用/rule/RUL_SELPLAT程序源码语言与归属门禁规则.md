...
uniform_architecture_upgrade_record = 2026-08-11:no_registry_structure_switch,no_project_name_bypass,table_business_plus_capability_plus_common_for_all_projects
...
selplat_nested_freeze_source_gate = applications,shared,java_generated_javascript,zero_nested_selFreeze
...
selplat_cross_platform_path_comparison = java.nio.file.Path,python.pathlib.Path,component_or_relative_suffix,no_fixed_separator_string_assertion
...
selplat_managed_database_application_detection = db_sql_directory|generated_project_ownership_marker|active_user_central_registry,central_registration_required_for_all
...
selplat_application_service_database_access = business_service_or_BaseDao_only,no_direct_JdbcTemplate_in_service,JdbcTemplate_allowed_only_in_persistence_or_migration
<!-- 查询表示按真实持久化模型归属；Type 维护分类目录，TreeNode 独立通过 code 与 parentId 建树，禁止跨表 DAO 或类型外键重新制造耦合。 -->
selplat_query_representation_controller_boundary = own_table_when_distinct_persistence_model,type_catalog_independent,tree_node_code_plus_parentId_independent,no_cross_table_dao,no_type_foreign_key,no_ui_name_driven_duplicate_table_business
...
selplat_quick_gate_entry = ./gradlew selplatQuickGate,source_ownership_static_database_sql_and_component_governance_gate,no_spring,no_formal_database_connection
