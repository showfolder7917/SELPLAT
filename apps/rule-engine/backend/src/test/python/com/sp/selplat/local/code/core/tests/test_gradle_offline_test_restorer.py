...
# 真实能力文件用于验证注册后的生产逻辑。
ABILITY_PATH = CODE_ROOT / "abilities" / "gradle_offline_test_restorer.py"
# 统一 executor 必须能够加载包含 dataclass 的生产能力。
EXECUTOR_PATH = CODE_ROOT / "executor.py"
...
def load_ability():
    ...
    spec = importlib.util.spec_from_file_location(
        "gradle_offline_test_restorer_under_test",
        ABILITY_PATH,
    )
    # 能力路径缺失时立即失败。
...
class GradleOfflineTestRestorerTests(unittest.TestCase):
    ...
    def test_registered_ability_loads_through_executor(self) -> None:
        """统一 executor 必须能够加载并执行登记后的 dataclass 能力。"""

        # executor 作为真实模块加载，验证注册表到能力入口的完整链路。
        executor_spec = importlib.util.spec_from_file_location(
            "gradle_offline_executor_under_test",
            EXECUTOR_PATH,
        )
        # 执行器路径缺失时立即失败。
        ...
        with tempfile.TemporaryDirectory(dir=TEST_TEMP_ROOT) as temp_directory:
            ...
            # 通过注册表和 executor 执行新能力。
            result = executor_module.execute_ability(
                "gradle_offline_test_restorer",
                {
                    "action": "plan",
                    "project_root": str(project_root),
            ...
            # 完整注册调用必须正常返回计划。
            self.assertEqual(result["status"], "completed")
            self.assertEqual(result["ability"], "gradle_offline_test_restorer")
