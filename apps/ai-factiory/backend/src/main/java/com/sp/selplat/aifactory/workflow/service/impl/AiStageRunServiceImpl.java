package com.sp.selplat.aifactory.workflow.service.impl;
...
import com.sp.selplat.aifactory.workflow.service.AiStageRunService;
...
public class AiStageRunServiceImpl implements AiStageRunService {
     ...
     * @param dao 控制面 DAO
     */
    public AiStageRunServiceImpl(AiFactoryControlDao dao) { this.dao = dao; }

    /** {@inheritDoc} */
    @Override
    public CommonResult claim(CommonParam command) {
        String token = UUID.randomUUID().toString();
        Map<String, Object> lease = dao.claimStage(required(command, "stageId"),
    ...
    public CommonResult complete(CommonParam command) {
        ...
        return AiFactoryResults.success(dao.completeRun(required(command, "runId"), exitCode, digests),
                "执行事实已接收，等待本地文件门禁。");
    }
