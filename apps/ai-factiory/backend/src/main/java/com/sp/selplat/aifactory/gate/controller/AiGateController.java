package com.sp.selplat.aifactory.gate.controller;
...
public class AiGateController {
    private final AiGateService service;

    /**
     * 注入 Gate Service。
     * 真实传参示例：Spring 注入 AiGateServiceImpl。
     * 真实返回示例：控制器可响应 evidence.htm。
     * 异常或副作用示例：缺少 Service 时启动失败；不运行门禁。
     * @param service Gate 业务服务
     */
    public AiGateController(AiGateService service) { this.service = service; }
