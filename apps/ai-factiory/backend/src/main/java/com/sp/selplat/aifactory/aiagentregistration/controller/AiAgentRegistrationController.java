package com.sp.selplat.aifactory.aiagentregistration.controller;
...
public class AiAgentRegistrationController {
    private final AiAgentRegistrationService service;

    /**
     * 注入 Agent 登记 Service。
     * 真实传参示例：Spring 注入 AiAgentRegistrationServiceImpl。
     * 真实返回示例：控制器可响应 resolve/state。
     * 异常或副作用示例：缺少 Service 时启动失败；不启动 Agent。
     * @param service Agent 登记服务
     */
    public AiAgentRegistrationController(AiAgentRegistrationService service) { this.service = service; }
