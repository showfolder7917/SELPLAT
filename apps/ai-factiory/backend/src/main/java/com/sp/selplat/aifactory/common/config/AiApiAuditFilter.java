...
public class AiApiAuditFilter extends OncePerRequestFilter {
    private final AiAuditEventService service;

    /**
     * 注入审计 Service。
     * 真实传参示例：Spring 注入 AiAuditEventServiceImpl。
     * 真实返回示例：过滤器可审计 /api/v1/ai-factory/**。
     * 异常或副作用示例：Service 缺失时启动失败；构造过程不读取请求。
     * @param service 服务端审计服务
     */
    public AiApiAuditFilter(AiAuditEventService service) { this.service = service; }
