package com.sp.selplat.aifactory.aiagentregistration.service.impl;

import com.sp.selplat.aifactory.aiagentregistration.service.AiAgentRegistrationService;
...
@Service
public class AiAgentRegistrationServiceImpl implements AiAgentRegistrationService {
     ...
     * @param dao 控制面 DAO
     */
    public AiAgentRegistrationServiceImpl(AiTaskService taskService) { this.taskService = taskService; }

    /** {@inheritDoc} */
    @Override
    public CommonResult getStageRole(CommonParam query) {
        Map<String, Object> role = new LinkedHashMap<>(taskService.findApprovedRole(required(query, "stageId")));
    ...
    public CommonResult resolve(CommonParam query) {
        Map<String, Object> agent = new LinkedHashMap<>(taskService.resolveAgent(required(query, "roleId"), required(query, "roleVersion")));
    ...
    public CommonResult reportState(CommonParam command) {
        ...
        taskService.appendAgentState(required(command, "runId"), required(command, "agentId"), sequence, required(command, "state"), digest);
