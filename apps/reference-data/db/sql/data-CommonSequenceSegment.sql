-- 七张业务表号段只在缺失时初始化；重启不覆盖管理员已经推进的 nextStartId 和 versionNo。
...
INSERT INTO CommonSequenceSegment
    (tenantId, lastOperateUserId, seqCode, seqName, nextStartId, stepSize, versionNo, remark, sortnum, status)
SELECT 1, 1, 'ReferenceDataControlBindingId', '页面控件绑定主键', 101000, 100, 0, 'Reference Data 页面控件绑定号段', 70, 1
WHERE NOT EXISTS (SELECT 1 FROM CommonSequenceSegment WHERE seqCode = 'ReferenceDataControlBindingId');
