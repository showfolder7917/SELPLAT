...
应用启动时按 Java 中登记的 `sql/*.sql` 固定顺序初始化结构。全新数据库只建立六张业务表、六个表主键号段和一个通用逻辑对象号段，
...
六张表分别使用 `<ActualTableName>Id` 独立号段，并由后端用“对象类型前缀 + 本表 id”生成不可变 code；
`ReferenceDataObjectId` 不再生成表主键，只用于 `optionSetCode` 等没有独立实体表的共享逻辑编码；
表格元素只通过真实外键 `tableId` 归属父表，公开定位只使用 code。

对象前缀固定为 `page/window/control/table/tableElement/type`；树节点固定使用 `treeNode`。
前缀便于人工识别；树节点只使用本表 `parentId` 建树，其他关联关系依靠
`tableId`、`pageCode` 以及 `parentKind + parentCode`，禁止解析前缀代替外键。
...
- 引用数据六表严格一表一号段；`ReferenceDataObjectId` 仅作为已登记的无实体表逻辑编码号段。
...
- 六张业务表初始化脚本不得写业务数据；只有各表号段和通用逻辑号段允许使用
