...
(function mdaInitializeApplication() {
    ...
    function mdaBuildPayload() {
        ...
        return Object.freeze({
            ...
            title: Object.freeze({
                ...
                actions: Object.freeze([Object.freeze({ id: "new", label: "SQL 查询", icon: "ri-terminal-box-line", primary: true })]),
            ...
            select: Object.freeze({
                projectType: Object.freeze({ gridId: mdaGridId, role: "type-filter", label: "数据库连接", ariaLabel: "选择数据库连接", currentTemplate: "{label}，当前：{value}", menuTitle: "选择数据库连接", prefix: "连接：", scrollAfter: 6, options: Object.freeze(mdaState.connections.map((connection) => Object.freeze({ value: String(connection.id), label: connection.connectionName, icon: "ri-database-2-line", description: connection.databaseType, selected: String(connection.id) === connectionId }))) }),
    ...
    async function mdaExecuteSql(sql) {
        ...
        window.selPanel.setLocale(window.selPanel.get(mdaGridId), { view: payload });
        return response;
    }
    ...
    async function mdaMountApplication() {
        ...
        if (!mdaState.treeController || !mdaState.gridController || !mdaState.sqlWindowController) throw new Error("MDA 公共业务组件挂载失败。");
        ...
        mdaApplicationHost.addEventListener("selWindow:submit", async (event) => {
            if (event.detail?.id !== "MdaSqlWindow") return;
